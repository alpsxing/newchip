...
int IBI2CControl::i2cget(int address, unsigned char inlen, unsigned char *inbuf, unsigned char length, unsigned char *values)
{
    ...
    (work_queue.msgs[0]).len = inlen;  
    (work_queue.msgs[0]).addr = address;  
    (work_queue.msgs[0]).buf = inbuf;  
  
    (work_queue.msgs[1]).len = length;  
    (work_queue.msgs[1]).flags = I2C_M_RD;  
    (work_queue.msgs[1]).addr = address;  
    (work_queue.msgs[1]).buf = values;  
  
    ret = ioctl(m_file, I2C_RDWR, (unsigned long) &work_queue);  
